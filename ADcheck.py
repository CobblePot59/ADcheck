...
class ADcheck:
    ...
    def domain_controlers(self, _return=False):
        ...
        for computer in self.computer_entries:
            if 'SERVER_TRUST_ACCOUNT' in uac_details(computer.userAccountControl.value):
    ...
    def accounts_never_expire(self):
        ...
        for user in self.user_entries:
            if 'DONT_EXPIRE_PASSWORD' in uac_details(user.userAccountControl.value):
                password_unexpire.append(user.samAccountName.value)
    ...
    def admin_can_be_delegated(self):
        result = []
        for user in self.user_entries:
            if hasattr(user, 'adminCount') and user.cn != 'krbtgt' and 'NOT_DELEGATED' not in uac_details(user.userAccountControl.value):
                result.append(user.samAccountName.value)
    ...
    def admin_not_protected(self):
        ...
        for user in self.user_entries:
            if hasattr(user, 'adminCount') and hasattr(user, 'memberOf') and user.cn != 'krbtgt':
    ...
    def ldap_signing(self):
        ...
        try:
            ADclient(domain=domain, username=username, password=password, dc_ip=dc_ip, base_dn=base_dn, secure=False)
    ...
    def privesc_group(self):
        ...
        for group in PRIVESC_GROUP:
            if self.ad_client.get_member(group):
    ...
    def reversible_password(self):
        result = []
        for user in self.user_entries:
            if 'ENCRYPTED_TEXT_PASSWORD_ALLOWED' in uac_details(user.userAccountControl.value):
                result.append(user.samAccountName.value)
    ...
    def locked_accounts(self):
        naccounts = []
        for user in self.user_entries:
            if 'LOCKOUT' in uac_details(user.userAccountControl.value):
                naccounts.append(user.samAccountName.value)
        result = False
        if len(naccounts) > 5:
            result = True
    ...
    def des_authentication(self):
        result = []
        for user in self.user_entries:
            if 'USE_DES_KEY_ONLY' in uac_details(user.userAccountControl.value):
                result.append(user.samAccountName.value)
    ...
    def asreproast(self):
        result = []
        for user in self.user_entries:
            if 'DONT_REQ_PREAUTH' in uac_details(user.userAccountControl.value):
                result.append(user.samAccountName.value)
    ...
    def kerberoast(self):
        result = []
        for user in self.user_entries:
            if hasattr(user, 'servicePrincipalName') and user.cn != 'krbtgt':
                result.append(user.samAccountName.value)
    ...
    def trusted_for_delegation(self):
        users = []
        for user in self.user_entries:
            if 'TRUSTED_FOR_DELEGATION' in uac_details(user.userAccountControl.value):
                users.append(user.samAccountName.value)

        computers = []
        for computer in self.computer_entries:
            if 'TRUSTED_FOR_DELEGATION' in uac_details(computer.userAccountControl.value) and not 'SERVER_TRUST_ACCOUNT'  in uac_details(computer.userAccountControl.value):
                computers.append(computer.samAccountName.value)
    ...
    def password_not_required(self):
        result = []
        for user in self.user_entries:
            if 'PASSWD_NOTREQD' in uac_details(user.userAccountControl.value):
                result.append(user.samAccountName.value)
    ...
    def was_admin(self):
        result = []
        for user in self.user_entries:
            if hasattr(user, 'adminCount') and user.cn != 'krbtgt' and user.cn != 'Administrator':
                result.append(user.samAccountName.value)
    ...
    def gpo_by_ou(self):
        ...
        for entry in self.all_entries:
            if hasattr(entry, 'gPLink'):
                groups.append({'dn': entry.distinguishedName.value, 'name': re.findall(r'{(.*?)}', entry.gPLink.value)})
    ...
    def force_logoff(self):
        result = False
        if self.root_entry.forceLogoff.value == 0:
            result = True
        ...
        for attribute in attributes:
            ...
            for user in self.user_entries:
                if hasattr(user, attribute):
                    users_attribute[attribute].append(user.samAccountName.value)
    ...
    def laps(self):
        try:
            ...
            result = False
            for computer in self.computer_entries:
                if hasattr(computer, 'msLAPS-PasswordExpirationTime'):
                    result = True
                    break
    ...
    def constrained_delegation(self):
        result = []
        for computer in self.computer_entries:
            if hasattr(computer, 'msDS-AllowedToDelegateTo'):
                result.append(f"{computer.samAccountName.value}: {computer['msDS-AllowedToDelegateTo'].value}")
        print_with_color(result, f'Computers with constrained delegation :\n{json.dumps(result, indent=4)}')

    def rbac(self):
        result = []
        for computer in self.computer_entries:
            if hasattr(computer, 'msDS-AllowedToActOnBehalfOfOtherIdentity'):
                result.append(computer.samAccountName.value)
    ...
    def gMSA(self):
        ...
        if gMSAs:
             for gMSA in gMSAs:
                result.append({'dn': gMSA.distinguishedName.value, 'msDS-HostServiceAccountBL': gMSA['msDS-HostServiceAccountBL'].value, 'msDS-ManagedPasswordInterval': gMSA['msDS-ManagedPasswordInterval'].value})
    ...
    def silos(self):
        ...
        for policy in authn_container:
            if hasattr(policy, 'msDS-AuthNPolicyEnforced'):
                authn_policies.append(policy)

        authn_silos = []
        for policy in authn_container:
            if hasattr(policy, 'msDS-AuthNPolicySiloEnforced'):
    ...
    def control_delegations(self):
        ...
        for container in containers:
            ...
            result = {}
            result.setdefault(container.distinguishedName.value, [])
            for user, permissions in user_permissions.items():
                ...
                result[container.distinguishedName.value] = permission_strings
    ...
    def bitlocker(self):
        ...
        if recovery_information:
            for computer in recovery_information:
                result.append(str(computer.distinguishedName.value).split(','))
            ...
            for line in open(file_path):
                if 'cpassword' in line:
                    ...
                    for policy in self.policies_entries:
                        if entry == policy.cn.value:
                            result.append(policy.displayName.value)
        print_with_color(result, f'Group Policy containing a password : {result}')

    def timeroast(self):
        computers_noLogonCount = [computer.samAccountName.value for computer in self.ad_client.get_ADobjects(custom_filter='(&(userAccountControl=4128)(logonCount=0))')]
        result = []
    ...
    def users_description(self):
        result = []
        for user in self.user_entries:
            if hasattr(user, 'description') and not self.NEW_WELL_KNOWN_SIDS.get(user.objectSid.value):
                result.append(user.samAccountName.value)
